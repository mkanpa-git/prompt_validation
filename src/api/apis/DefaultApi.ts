...
import {
    ApplicationSubmissionFromJSON,
    ApplicationSubmissionToJSON,
} from '../models/index';

export interface ApplApplIdGetRequest {
    applId: string;
}
...
export class DefaultApi extends runtime.BaseAPI {
     ...
     * Get Application Data By ID
     */
    async applApplIdGetRaw(requestParameters: ApplApplIdGetRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<ApplicationSubmission>> {
        if (requestParameters['applId'] == null) {
            throw new runtime.RequiredError(
                'applId',
                'Required parameter "applId" was null or undefined when calling applApplIdGet().'
            );
        }
        ...
        if (this.configuration && this.configuration.accessToken) {
            // oauth required
            headerParameters["Authorization"] = await this.configuration.accessToken("password", ["read", "write"]);
        }

        const response = await this.request({
            path: `/appl/{applId}`.replace(`{${"applId"}}`, encodeURIComponent(String(requestParameters['applId']))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        }, initOverrides);

        return new runtime.JSONApiResponse(response, (jsonValue) => ApplicationSubmissionFromJSON(jsonValue));
    }
     ...
     * Get Application Data By ID
     */
    async applApplIdGet(requestParameters: ApplApplIdGetRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<ApplicationSubmission> {
        const response = await this.applApplIdGetRaw(requestParameters, initOverrides);
